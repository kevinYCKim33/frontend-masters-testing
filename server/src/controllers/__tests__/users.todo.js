...
// setup1 setup2 works too...doesn't have to be just one...
// object you need...state to initiate...
// beforeEach works...let rec...
// don't like using those...Kent feels it obscures...

// any setup that's common to all our tests...
function setup() {
  const req = {
    body: {},
  }
...
// TDDing things up!
// let's make sure the person deleting is authenticated admin!
...
test('deleteUser will 403 if not made by the author', async () => {
  const testUser = await db.insertUser(generate.userData()) //user you're trying to delete
  const {req, res} = setup()
  req.params = {id: testUser.id}
